...
impl<R> SyntaxExt for R where R: Syntax {}
...
mod tests {
    ...
    #[test]
    fn comma_delimited() {
        let mut parser = Lexer::lex("select row_1, row_2, row_3 from my_table where row_id > 0;").unwrap();

        parser.pop().unwrap();
        let v = columns::Column::parse_comma_delimited(&mut parser).unwrap();
        let correct = vec![Token::Identifier("row_1".into()), Token::Identifier("row_2".into()), Token::Identifier("row_3".into())];
